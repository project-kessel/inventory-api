...
func (m *ReporterData) validate(all bool) error {
	...
	if utf8.RuneCountInString(m.GetSubjectLocalResourceId()) < 1 {
		err := ReporterDataValidationError{
			field:  "SubjectLocalResourceId",
	...
	if utf8.RuneCountInString(m.GetObjectLocalResourceId()) < 1 {
		err := ReporterDataValidationError{
			field:  "ObjectLocalResourceId",
			reason: "value length must be at least 1 runes",
		}
		if !all {
			return err
