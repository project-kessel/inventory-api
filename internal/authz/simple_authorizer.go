...
// ReadTuples implements Authorizer.
// It returns tuples matching the filter.
// This is a simple direct-tuple lookup, not a full ReBAC graph traversal.
func (s *SimpleAuthorizer) ReadTuples(_ context.Context, req *kessel.ReadTuplesRequest) (grpc.ServerStreamingClient[kessel.ReadTuplesResponse], error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	...
	for key := range s.tuples {
		if filter == nil || matchesFilter(key, filter) {
