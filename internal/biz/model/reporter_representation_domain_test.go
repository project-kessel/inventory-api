...
func TestReporterRepresentation_FactoryMethods(t *testing.T) {
	t.Run("should_create_valid_ReporterRepresentation_using_factory", func(t *testing.T) {
		...
		// UUID is generated by BeforeCreate hook, so it will be nil until saved to database
		if rr.ID != uuid.Nil {
			t.Error("Expected nil UUID from factory method, UUID generation happens in BeforeCreate hook")
		}

		// Verify UUID will be set in metadata foreign key by BeforeCreate hook
		if rr.Metadata.ReporterRepresentationID != uuid.Nil {
			t.Error("Expected nil UUID in metadata foreign key, will be set by BeforeCreate hook")
		}
	...
	t.Run("should_generate_UUID_in_BeforeCreate_hook", func(t *testing.T) {
		rr, err := NewReporterRepresentation(
			JsonObject{"satellite_id": "test-satellite"},
			"local-123",
			"hbi",
		...
		// Simulate BeforeCreate hook call
		err = rr.BeforeCreate(nil)
		if err != nil {
			t.Fatalf("BeforeCreate hook should not fail, got %v", err)
		}

		// Verify UUID is generated
		if rr.ID == uuid.Nil {
			t.Error("Expected non-nil UUID after BeforeCreate hook")
		}

		// Verify UUID is set in metadata foreign key
		if rr.Metadata.ReporterRepresentationID != rr.ID {
			t.Errorf("Expected metadata foreign key to match ID, got %v vs %v", rr.Metadata.ReporterRepresentationID, rr.ID)
		}
	})

	t.Run("should_not_overwrite_existing_UUID_in_BeforeCreate_hook", func(t *testing.T) {
		rr, err := NewReporterRepresentation(
			JsonObject{"satellite_id": "test-satellite"},
			"local-123",
			"hbi",
		...
		// Verify UUID is not overwritten
		if rr.ID != existingUUID {
			t.Errorf("Expected UUID to remain unchanged, got %v, expected %v", rr.ID, existingUUID)
		}

		// Verify UUID is NOT set in metadata foreign key when ID already exists
		if rr.Metadata.ReporterRepresentationID != uuid.Nil {
			t.Errorf("Expected metadata foreign key to remain nil when ID already exists, got %v", rr.Metadata.ReporterRepresentationID)
		}
	})
