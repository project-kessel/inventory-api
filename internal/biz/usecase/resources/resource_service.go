...
func (uc *Usecase) ReportResource(ctx context.Context, cmd ReportResourceCommand) error {
	...
	err = reportResource()
	if err != nil && isDuplicateTransactionError(err) {
		if uc.Config.IdempotencyCheckEnabled {
			// Race: the advisory check passed but a concurrent request committed the
			// same transaction ID before our write completed. The unique constraint
			// caught it — treat as idempotent success.
			log.Debugf("Concurrent duplicate transaction ID detected, treating as idempotent: %s", cmd.TransactionId.String())
			return nil
		}
		// Idempotency check disabled: retry with a fresh transaction ID.
