...
// noopOutboxPublisher returns a no-op OutboxPublisher for use with SQLite in tests.
func noopOutboxPublisher() OutboxPublisher { return SetOutboxPublisher(storage.OutboxModeNone) }
...
func TestEmptyAndNilRepresentationPersistence(t *testing.T) {
	...
	saveAndFind := func(t *testing.T, p resourceParams, reporter, common *bizmodel.Representation, txSuffix string) *bizmodel.Resource {
		t.Helper()
		db := setupInMemoryDB(t)
		repo := newTestGormResourceRepository(db)
		...
		err = repo.Transact("test", func(tx bizmodel.ResourceTx) error {
			return tx.Save(resource, bizmodel.OperationTypeCreated, emptyTxId)
		})
		require.NoError(t, err)
	...
	t.Run("nil reporter representation with valid common stores successfully", func(t *testing.T) {
		...
		found := saveAndFind(t, p, nil, validCommon(t, "ws-nil-reporter"), "nil-reporter-test")
	...
	t.Run("valid reporter with nil common representation stores successfully", func(t *testing.T) {
		...
		found := saveAndFind(t, p, validReporter(t), nil, "nil-common-test")
	...
	t.Run("empty reporter representation with valid common stores successfully", func(t *testing.T) {
		...
		found := saveAndFind(t, p, emptyRep(), validCommon(t, "ws-empty-reporter"), "empty-reporter-test")
