...
func newTestGormResourceRepository(t *testing.T, db *gorm.DB) bizmodel.Store {
	t.Helper()
...
func TestResourceRepositoryContract(t *testing.T) {
	implementations := []struct {
	...
	}{
		{
			...
			store: func() bizmodel.Store {
				...
				return newTestGormResourceRepository(t, db)
			},
			db: func() *gorm.DB {
	...
	for _, impl := range implementations {
		t.Run(impl.name, func(t *testing.T) {
			testRepositoryContract(t, impl.store(), impl.db())
		})
	}
}

func testRepositoryContract(t *testing.T, store bizmodel.Store, db *gorm.DB) {
	tx, err := store.Begin()
	...
	t.Run("Save and FindResourceByKeys basic workflow", func(t *testing.T) {
		...
		err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("contract-tx-1"))
		require.NoError(t, err, "Save should succeed")

		key := createContractReporterResourceKey(t, "contract-test-1", "k8s_cluster", "ocm", "ocm-instance-1")

		foundResource, err := repo.FindResourceByKeys(key)
	...
	t.Run("FindResourceByKeys returns ErrRecordNotFound for non-existent", func(t *testing.T) {
		...
		foundResource, err := repo.FindResourceByKeys(key)
		require.ErrorIs(t, err, bizmodel.ErrResourceNotFound, "Should return ErrRecordNotFound")
	...
	t.Run("Save-Update-Save workflow", func(t *testing.T) {
		...
		err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("contract-tx-create"))
		...
		err = repo.Save(*foundResource, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId("contract-tx-update"))
		require.NoError(t, err, "Updated save should succeed")

		// Verify update persisted
		updatedResource, err := repo.FindResourceByKeys(key)
	...
	t.Run("Save-Delete workflow", func(t *testing.T) {
		...
		err = repo.Save(*foundResource, bizmodel.OperationTypeDeleted, bizmodel.NewTransactionId("contract-tx-delete"))
		require.NoError(t, err, "Delete save should succeed")

		// Verify deletion behavior is consistent
		deletedResource, err := repo.FindResourceByKeys(key)
		if err == bizmodel.ErrResourceNotFound {
	...
	t.Run("Unique constraint enforcement", func(t *testing.T) {
		...
		err := repo.Save(resource1, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("contract-tx-1"))
		...
		err = repo.Save(resource2, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("contract-tx-2"))
	...
	t.Run("Case insensitive key matching for non ID fields", func(t *testing.T) {
		...
		err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("contract-case-tx"))
		require.NoError(t, err, "Save should succeed")
	...
	t.Run("Transaction handling", func(t *testing.T) {
		...
		err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("contract-tx"))
		require.NoError(t, err, "Save within transaction should succeed")

		key := createContractReporterResourceKey(t, "contract-tx-test", "k8s_cluster", "ocm", "ocm-instance-1")

		foundResource, err := repo.FindResourceByKeys(key)
		require.NoError(t, err, "Find within transaction should succeed")
		require.NotNil(t, foundResource)
	})
	...
	t.Run("Lifecycle: Create-Update-Delete-Recreate", func(t *testing.T) {
		...
		err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("contract-create"))
		...
		err = repo.Save(*foundResource, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId("contract-update"))
		...
		err = repo.Save(*deletedResource, bizmodel.OperationTypeDeleted, bizmodel.NewTransactionId("contract-delete"))
		require.NoError(t, err, "Delete save should succeed")

		// 4. Verify delete behavior
		postDeleteResource, err := repo.FindResourceByKeys(key)
		...
		err = repo.Save(newResource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("contract-recreate"))

		// The behavior should be identical between implementations
		recreateResource, findErr := repo.FindResourceByKeys(key)
		if err == nil {
			// Recreate succeeded
	...
	// Helper to build a resource with or without common rep for use inside contract tests.
	makeResource := func(t *testing.T, localID, txPrefix string, withCommon bool) (bizmodel.Resource, bizmodel.ReporterResourceKey) {
		t.Helper()
		rID, _ := bizmodel.NewResourceId(uuid.New())
		rrID, _ := bizmodel.NewReporterResourceId(uuid.New())
		lid, _ := bizmodel.NewLocalResourceId(localID)
		rt, _ := bizmodel.NewResourceType("k8s_cluster")
		rpt, _ := bizmodel.NewReporterType("ocm")
		rinst, _ := bizmodel.NewReporterInstanceId("ocm-instance-1")
	...
	doUpdate := func(t *testing.T, found *bizmodel.Resource, key bizmodel.ReporterResourceKey, txPrefix string, withCommon bool) {
		t.Helper()
	...
	t.Run("CommonVersion lifecycle consistency", func(t *testing.T) {
		t.Run("create-with update-with update-without: increments then resets to nil", func(t *testing.T) {
			resource, key := makeResource(t, "cv-contract-inc-reset", "cv-inc-reset", true)
			require.NoError(t, repo.Save(resource, bizmodel.OperationTypeCreated, emptyTxId))

			found, err := repo.FindResourceByKeys(key)
			require.NoError(t, err)
			snap, _, _, _, err := found.Serialize()
			require.NoError(t, err)
			assert.Equal(t, uint(0), *snap.CommonVersion, "should be 0 after create-with")

			doUpdate(t, found, key, "cv-inc-reset-upd1", true)
			require.NoError(t, repo.Save(*found, bizmodel.OperationTypeUpdated, emptyTxId))

			found, err = repo.FindResourceByKeys(key)
			require.NoError(t, err)
			snap, _, _, _, err = found.Serialize()
			require.NoError(t, err)
			assert.Equal(t, uint(1), *snap.CommonVersion, "should be 1 after update-with")

			doUpdate(t, found, key, "cv-inc-reset-upd2", false)
			require.NoError(t, repo.Save(*found, bizmodel.OperationTypeUpdated, emptyTxId))
			...
			snap, _, _, _, err = found.Serialize()
			require.NoError(t, err)
			assert.Nil(t, snap.CommonVersion, "should be nil after update-without")
		})

		t.Run("create-with update-without update-with: drop then re-add resumes from last version", func(t *testing.T) {
			resource, key := makeResource(t, "cv-contract-drop-readd", "cv-drop-readd", true)
			require.NoError(t, repo.Save(resource, bizmodel.OperationTypeCreated, emptyTxId))

			found, err := repo.FindResourceByKeys(key)
			require.NoError(t, err)
			doUpdate(t, found, key, "cv-drop-readd-upd1", false)
			require.NoError(t, repo.Save(*found, bizmodel.OperationTypeUpdated, emptyTxId))
			...
			doUpdate(t, found, key, "cv-drop-readd-upd2", true)
			require.NoError(t, repo.Save(*found, bizmodel.OperationTypeUpdated, emptyTxId))
			...
			snap, _, _, _, err = found.Serialize()
			require.NoError(t, err)
			require.NotNil(t, snap.CommonVersion, "should be set after re-add")
			assert.Equal(t, uint(1), *snap.CommonVersion, "re-add should resume at 1, not restart at 0")
		})

		t.Run("create-without update-with: first-time add via update initializes to 0", func(t *testing.T) {
			resource, key := makeResource(t, "cv-contract-first-add", "cv-first-add", false)
			require.NoError(t, repo.Save(resource, bizmodel.OperationTypeCreated, emptyTxId))
			...
			snap, _, _, _, err := found.Serialize()
			require.NoError(t, err)
			assert.Nil(t, snap.CommonVersion, "should be nil after create-without")

			doUpdate(t, found, key, "cv-first-add-upd1", true)
			require.NoError(t, repo.Save(*found, bizmodel.OperationTypeUpdated, emptyTxId))
			...
			snap, _, _, _, err = found.Serialize()
			require.NoError(t, err)
			require.NotNil(t, snap.CommonVersion)
			assert.Equal(t, uint(0), *snap.CommonVersion, "first-ever common rep via update should initialize to 0")
		})

		t.Run("create-with delete create-with update-with: second lifecycle starts fresh", func(t *testing.T) {
			resource1, key := makeResource(t, "cv-contract-del-recreate", "cv-del-rec-1", true)
			require.NoError(t, repo.Save(resource1, bizmodel.OperationTypeCreated, emptyTxId))

			found, err := repo.FindResourceByKeys(key)
			require.NoError(t, err)
			doUpdate(t, found, key, "cv-del-rec-upd1", true)
			require.NoError(t, repo.Save(*found, bizmodel.OperationTypeUpdated, emptyTxId))

			found, err = repo.FindResourceByKeys(key)
			require.NoError(t, err)
			require.NoError(t, found.Delete(key))
			require.NoError(t, repo.Save(*found, bizmodel.OperationTypeDeleted, emptyTxId))

			resource2, _ := makeResource(t, "cv-contract-del-recreate", "cv-del-rec-2", true)
			require.NoError(t, repo.Save(resource2, bizmodel.OperationTypeCreated, emptyTxId))

			found, err = repo.FindResourceByKeys(key)
			require.NoError(t, err)
			snap, _, _, _, err := found.Serialize()
			require.NoError(t, err)
			assert.Equal(t, uint(0), *snap.CommonVersion, "second lifecycle should start at 0")

			doUpdate(t, found, key, "cv-del-rec-upd2", true)
			require.NoError(t, repo.Save(*found, bizmodel.OperationTypeUpdated, emptyTxId))
			...
			snap, _, _, _, err = found.Serialize()
			require.NoError(t, err)
			assert.Equal(t, uint(1), *snap.CommonVersion, "second lifecycle update should reach 1, not be influenced by first lifecycle")
		})
	})
...
func TestFindResourceByKeys(t *testing.T) {
	...
	for _, impl := range implementations {
		t.Run(impl.name, func(t *testing.T) {
			...
			getFreshInstances := func() (bizmodel.ResourceRepository, bizmodel.Tx, *gorm.DB) {
				if impl.name == "Fake Repository" || impl.name == "Fake Store" {
					store := impl.store()
					tx, err := store.Begin()
					require.NoError(t, err)
					return tx.ResourceRepository(), tx, impl.db()
				}
				db := setupInMemoryDB(t)
				store := newTestGormResourceRepository(t, db)
				tx, err := store.Begin()
				require.NoError(t, err)
				return tx.ResourceRepository(), tx, db
			}

			t.Run("Save and FindResourceByKeys workflow", func(t *testing.T) {
				repo, tx, _ := getFreshInstances()
				defer func() { _ = tx.Rollback() }()

				resource := createTestResource(t)
				err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("test-tx-123"))
				require.NoError(t, err)
			...
			t.Run("FindResourceByKeys with different keys returns different resources", func(t *testing.T) {
				...
				err := repo.Save(resource1, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("test-tx-1"))
				require.NoError(t, err)
				err = repo.Save(resource2, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("test-tx-2"))
				require.NoError(t, err)
				...
				found1, err := repo.FindResourceByKeys(key1)
				require.NoError(t, err)
				require.NotNil(t, found1)

				found2, err := repo.FindResourceByKeys(key2)
				require.NoError(t, err)
			...
			t.Run("FindResourceByKeys works with nil transaction", func(t *testing.T) {
				...
				resource := createTestResource(t)
				err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("test-tx-1"))
				require.NoError(t, err)
			...
			t.Run("FindResourceByKeys works when reporterInstanceId is not provided in search key", func(t *testing.T) {
				...
				err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("test-tx-no-instance"))
				require.NoError(t, err)
			...
			t.Run("Case-insensitive matching", func(t *testing.T) {
				...
				err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("test-tx-case"))
				require.NoError(t, err)
...
func TestFindResourceByKeys_TombstoneFilter(t *testing.T) {
	...
	for _, impl := range implementations {
		t.Run(impl.name, func(t *testing.T) {
			...
			err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("test-tx-tombstone"))
			require.NoError(t, err)
			...
			err = repo.Save(*foundResource, bizmodel.OperationTypeDeleted, bizmodel.NewTransactionId("test-tx-delete"))
			require.NoError(t, err)
	...
	for _, impl := range implementations {
		t.Run(impl.name, func(t *testing.T) {
			...
			t.Run("should allow same key with different versions", func(t *testing.T) {
				...
				err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("test-tx-create"))
				require.NoError(t, err, "Initial save should succeed")
				...
				err = repo.Save(resource, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId("test-tx-update"))
	...
	for _, impl := range implementations {
		t.Run(impl.name, func(t *testing.T) {
			...
			t.Run("report -> delete -> resubmit same delete", func(t *testing.T) {
				...
				resource := createTestResourceWithLocalId(t, localResourceId)
				err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("repo-create-1"))
				require.NoError(t, err, "Initial save should succeed")

				key := createContractReporterResourceKey(t, localResourceId, "k8s_cluster", "ocm", "ocm-instance-1")

				// Verify initial state
				afterCreate, err := repo.FindResourceByKeys(key)
				...
				err = repo.Save(*foundResource, bizmodel.OperationTypeDeleted, bizmodel.NewTransactionId("repo-delete-1"))
				require.NoError(t, err, "Delete save should succeed")

				// Verify delete state
				afterDelete1, err := repo.FindResourceByKeys(key)
				if err == bizmodel.ErrResourceNotFound {
				...
				foundResource2, err := repo.FindResourceByKeys(key)
				if err == bizmodel.ErrResourceNotFound {
					...
					err = repo.Save(*foundResource2, bizmodel.OperationTypeDeleted, bizmodel.NewTransactionId("repo-delete-2"))
					require.NoError(t, err, "Duplicate delete save should succeed")

					// Verify state after duplicate delete
					afterDelete2, err := repo.FindResourceByKeys(key)
					if err != bizmodel.ErrResourceNotFound {
			...
			t.Run("report -> resubmit same report -> delete -> resubmit same delete", func(t *testing.T) {
				...
				err := repo.Save(resource1, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("repo-create-1"))
				require.NoError(t, err, "Initial save should succeed")

				key := createContractReporterResourceKey(t, localResourceId, "k8s_cluster", "ocm", "ocm-instance-1")

				// 2. RESUBMIT SAME REPORT: Should succeed and increment version
				foundResource1, err := repo.FindResourceByKeys(key)
				...
				err = repo.Save(*foundResource1, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId("repo-update-1"))
				...
				err = repo.Save(*foundResource2, bizmodel.OperationTypeDeleted, bizmodel.NewTransactionId("repo-delete-1"))
				require.NoError(t, err, "Delete save should succeed")

				// 4. RESUBMIT SAME DELETE: Should succeed
				foundResource3, err := repo.FindResourceByKeys(key)
				if err == bizmodel.ErrResourceNotFound {
					// With tombstone filter, we expect this behavior
					t.Log("Cannot resubmit delete - resource not found due to tombstone filter (expected)")
				} else {
					...
					err = repo.Save(*foundResource3, bizmodel.OperationTypeDeleted, bizmodel.NewTransactionId("repo-delete-2"))
					require.NoError(t, err, "Duplicate delete save should succeed")
				}
			})
			...
			t.Run("complex idempotency: multiple report and delete cycles", func(t *testing.T) {
				...
				for cycle := 0; cycle < 3; cycle++ {
					...
					if err == bizmodel.ErrResourceNotFound {
						...
						resource := createTestResourceWithLocalId(t, localResourceId)
						err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId(fmt.Sprintf("repo-cycle-%d-create", cycle)))
						...
						err = repo.Save(*foundResource, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId(fmt.Sprintf("repo-cycle-%d-update", cycle)))
						require.NoError(t, err, "Update save should succeed in cycle %d", cycle)
					}

					// Verify current state after report/update
					currentResource, err := repo.FindResourceByKeys(key)
					...
					err = repo.Save(*currentResource, bizmodel.OperationTypeDeleted, bizmodel.NewTransactionId(fmt.Sprintf("repo-cycle-%d-delete", cycle)))
	...
	for _, impl := range implementations {
		t.Run(impl.name, func(t *testing.T) {
			...
			t.Run("Save handles duplicate calls gracefully", func(t *testing.T) {
				...
				err = repo.Save(resource, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId("test-tx-2"))
				require.NoError(t, err)
			...
			t.Run("Save creates new resource successfully", func(t *testing.T) {
				...
				err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("test-tx-save"))
				require.NoError(t, err)
			...
			t.Run("Save skips representations with zero value primary keys", func(t *testing.T) {
				...
				err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("test-tx-zero-pk"))
...
func TestResourceRepository_MultipleHostsLifecycle(t *testing.T) {
	...
	for _, impl := range implementations {
		t.Run(impl.name, func(t *testing.T) {
			...
			err := repo.Save(host1, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("tx-create-host1"))
			require.NoError(t, err, "Should create host1")

			err = repo.Save(host2, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("tx-create-host2"))
			...
			foundHost1, err := repo.FindResourceByKeys(key1)
			require.NoError(t, err, "Should find host1 after creation")
			require.NotNil(t, foundHost1)

			foundHost2, err := repo.FindResourceByKeys(key2)
			...
			err = repo.Save(*foundHost1, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId("tx-update-host1"))
			require.NoError(t, err, "Should save updated host1")

			err = repo.Save(*foundHost2, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId("tx-update-host2"))
			require.NoError(t, err, "Should save updated host2")

			// Verify both updated hosts can still be found
			updatedHost1, err := repo.FindResourceByKeys(key1)
			require.NoError(t, err, "Should find host1 after update")
			require.NotNil(t, updatedHost1)

			updatedHost2, err := repo.FindResourceByKeys(key2)
			...
			err = repo.Save(*updatedHost1, bizmodel.OperationTypeDeleted, bizmodel.NewTransactionId("tx-delete-host1"))
			require.NoError(t, err, "Should save deleted host1")

			err = repo.Save(*updatedHost2, bizmodel.OperationTypeDeleted, bizmodel.NewTransactionId("tx-delete-host2"))
	...
	for _, impl := range implementations {
		t.Run(impl.name, func(t *testing.T) {
			...
			t.Run("Report resource with just reporter data and no common data", func(t *testing.T) {
				...
				err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("tx-reporter-only"))
			...
			t.Run("Report resource with no reporter data but has common data", func(t *testing.T) {
				...
				err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("tx-common-only"))
			...
			t.Run("Report resource with both data, then just reporter data, then just common data", func(t *testing.T) {
				...
				err := repo.Save(resourceBoth, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("tx-both"))
				...
				err = repo.Save(*foundResource, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId("tx-reporter-update"))
				...
				err = repo.Save(*foundResource, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId("tx-common-update"))
				require.NoError(t, err, "Should save resource with common-only update")

				// Verify final resource can still be found
				finalResource, err := repo.FindResourceByKeys(key)
...
func TestSerializableCreateFails(t *testing.T) {
	t.Run("Real Repository with GormStore", func(t *testing.T) {
		db := setupInMemoryDB(t)
		store := newTestGormResourceRepository(t, db)
		...
		conflictGormTx := db.Begin(&sql.TxOptions{Isolation: sql.LevelSerializable})
		conflictRepo := &txResourceRepository{
			gormTx:          conflictGormTx,
			outboxPublisher: noopOutboxPublisher,
		}
		// Do a read to simulate how a service would before creating
		foundResource, err := conflictRepo.FindResourceByKeys(resource.ReporterResources()[0].ReporterResourceKey)
		assert.NotNil(t, err)
		assert.Nil(t, foundResource)
		assert.NoError(t, conflictRepo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("tx-conflict")))
		// Do NOT commit yet to hold locks

		// Attempt to create the same resource via a separate serializable transaction managed by Store
		err = store.RunSerializable("test_create_resource", func(tx bizmodel.Tx) error {
			repo := tx.ResourceRepository()
			foundResource, err := repo.FindResourceByKeys(resource.ReporterResources()[0].ReporterResourceKey)
			assert.NotNil(t, err)
			assert.Nil(t, foundResource)
			return repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("tx-create"))
		})
		assert.Error(t, err)
		assert.ErrorContains(t, err, "transaction failed")

		// Cleanup
		assert.NoError(t, conflictGormTx.Commit().Error)
	})
}
...
func TestSerializableUpdateFails(t *testing.T) {
	t.Run("Real Repository with GormStore", func(t *testing.T) {
		...
		initTx, err := store.Begin()
		require.NoError(t, err)
		initRepo := initTx.ResourceRepository()
		resource := createTestResourceWithLocalId(t, "serializable-update-conflict")
		require.NoError(t, initRepo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("tx-initial")))
		require.NoError(t, initTx.Commit())
		...
		assert.NoError(t, conflictRepo.Save(resource, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId("tx-conflict")))
		// Do NOT commit yet to hold locks

		// Attempt to update the same resource via Store-managed serializable transaction
		err = store.RunSerializable("test_update_resource", func(tx bizmodel.Tx) error {
			return tx.ResourceRepository().Save(resource, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId("tx-update"))
		})
		assert.Error(t, err)
		assert.ErrorContains(t, err, "transaction failed")
...
func TestFindLatestRepresentations(t *testing.T) {
	implementations := []struct {
		...
		{
			name:  "Fake Repository",
			store: func() bizmodel.Store { return NewFakeResourceRepository() },
	...
	for _, impl := range implementations {
		t.Run(impl.name, func(t *testing.T) {
			...
			getFresh := func() (bizmodel.ResourceRepository, bizmodel.Tx, *gorm.DB) {
				if impl.name == "Fake Repository" || impl.name == "Fake Store" {
					store := impl.store()
					tx, err := store.Begin()
					require.NoError(t, err)
			...
			err = repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("tx-latest-v0"))
			require.NoError(t, err)
			...
			require.NoError(t, err)
			err = repo.Save(resource, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId("tx-latest-v1"))
			require.NoError(t, err)
			...
			require.NoError(t, err)
			err = repo.Save(resource, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId("tx-latest-v2"))
			require.NoError(t, err)

			// Test FindLatestRepresentations
			result, err := repo.FindLatestRepresentations(key)
			require.NoError(t, err)
	...
	for _, impl := range implementations {
		t.Run(impl.name, func(t *testing.T) {
			...
			t.Run("GetCurrentAndPreviousWorkspaceID extracts workspace IDs correctly", func(t *testing.T) {
				...
				err := repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("tx-ws-test"))
				require.NoError(t, err)
				...
				require.NoError(t, err)
				require.NoError(t, repo.Save(resource, bizmodel.OperationTypeUpdated, bizmodel.NewTransactionId("tx-ws-update")))

				// Get current and previous versions
				version := bizmodel.NewVersion(1)
				cur, prev, err := repo.FindCurrentAndPreviousVersionedRepresentations(key, &version, bizmodel.OperationTypeUpdated)
				require.NoError(t, err)
			...
			t.Run("GetCurrentAndPreviousWorkspaceID handles version 0", func(t *testing.T) {
				...
				err = repo.Save(resource, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("tx-v0-test"))
				require.NoError(t, err)

				// Get version 0 representations
				version := bizmodel.NewVersion(0)
				cur, prev, err := repo.FindCurrentAndPreviousVersionedRepresentations(key, &version, bizmodel.OperationTypeCreated)
				require.NoError(t, err)
...
func TestHasTransactionIdBeenProcessed(t *testing.T) {
	...
	for _, impl := range implementations {
		t.Run(impl.name, func(t *testing.T) {
			testHasTransactionIdBeenProcessed(t, impl.store(), impl.db())
		})
	}
}

func testHasTransactionIdBeenProcessed(t *testing.T, store bizmodel.Store, db *gorm.DB) {
	tx, err := store.Begin()
	require.NoError(t, err)
	defer func() { _ = tx.Rollback() }()
	repo := tx.ResourceRepository()
	t.Run("Empty transaction ID returns false", func(t *testing.T) {
		processed, err := repo.HasTransactionIdBeenProcessed(emptyTxId)
		require.NoError(t, err)
	...
	t.Run("Non-existent transaction ID returns false", func(t *testing.T) {
		...
		processed, err := repo.HasTransactionIdBeenProcessed(bizmodel.NewTransactionId(transactionId))
		require.NoError(t, err)
	...
	t.Run("Transaction ID tracking for fake repository", func(t *testing.T) {
		...
		if fakeRepo, ok := repo.(*fakeResourceRepo); ok {
			transactionId := "test-transaction-456"

			// Initially should not be processed
			processed, err := fakeRepo.HasTransactionIdBeenProcessed(bizmodel.NewTransactionId(transactionId))
			require.NoError(t, err)
			...
			processed, err = fakeRepo.HasTransactionIdBeenProcessed(bizmodel.NewTransactionId(differentTransactionId))
			require.NoError(t, err)
	...
	t.Run("Real repository basic functionality", func(t *testing.T) {
		...
		if _, ok := repo.(*txResourceRepository); ok {
	...
	t.Run("Multiple transaction IDs are tracked independently", func(t *testing.T) {
		...
		processed1, err := repo.HasTransactionIdBeenProcessed(bizmodel.NewTransactionId(transactionId1))
		require.NoError(t, err)
		assert.False(t, processed1, "Transaction ID 1 should not be processed initially")

		processed2, err := repo.HasTransactionIdBeenProcessed(bizmodel.NewTransactionId(transactionId2))
		require.NoError(t, err)
		assert.False(t, processed2, "Transaction ID 2 should not be processed initially")

		processed3, err := repo.HasTransactionIdBeenProcessed(bizmodel.NewTransactionId(transactionId3))
		require.NoError(t, err)
...
func TestTransactionIDUniqueConstraint(t *testing.T) {
	...
	for _, impl := range implementations {
		t.Run(impl.name, func(t *testing.T) {
			testTransactionIDUniqueConstraint(t, impl.store(), impl.db())
		})
	}
}

func testTransactionIDUniqueConstraint(t *testing.T, store bizmodel.Store, db *gorm.DB) {
	tx, err := store.Begin()
	require.NoError(t, err)
	defer func() { _ = tx.Rollback() }()
	repo := tx.ResourceRepository()
	t.Run("should enforce unique TransactionID constraint on duplicate operations", func(t *testing.T) {
		...
		err = repo.Save(resource1, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("tx-duplicate-1"))
		require.NoError(t, err, "First save should succeed")
		...
		err = repo.Save(resource2, bizmodel.OperationTypeCreated, bizmodel.NewTransactionId("tx-duplicate-2"))
...
func TestCommonVersionIncrementAndResetCycle(t *testing.T) {
	...
	err = repo.Save(resource, bizmodel.OperationTypeCreated, emptyTxId)
	require.NoError(t, err)
	...
	err = repo.Save(*found, bizmodel.OperationTypeUpdated, emptyTxId)
	require.NoError(t, err)
...
func TestNullCommonVersionPersistence(t *testing.T) {
	...
	retrievedResource, err := repo.FindResourceByKeys(key)
