...
func (r *Repo) Create(ctx context.Context, m *model.Resource, namespace string, txid string) (*model.Resource, error) {
	...
	err := handleSerializableTransaction(db, func(tx *gorm.DB) error {
...
func handleSerializableTransaction(db *gorm.DB, txFunc func(tx *gorm.DB) error) error {
	var err error
	for i := 0; i < maxRetries; i++ {
		...
		err = txFunc(tx)
		if err != nil {
			log.Debugf("transaction failed before commit (attempt %d/%d): %v", i+1, maxRetries, err)
			tx.Rollback()
			continue
		}
		err = tx.Commit().Error
		if err != nil {
			log.Debugf("error committing transaction (attempt %d/%d): %v", i+1, maxRetries, err)
			tx.Rollback()
			continue
		}
		return nil
